...
{
  //log_info(LD_REND,desc_id);//shuai test,the desc_id is the second column 
  char service_id[REND_SERVICE_ID_LEN_BASE32+1];//add by yinshaui
  time_t now = time(NULL);//add by yinshuai
  /*yinshuai in this place we shall modify something */
  ...
  if (e) {
    *desc = e->desc;
	 // yinshuai next
	//log_info(LD_REND, e->parsed->pk);//yinshuai add test 
	if (rend_get_service_id(e->parsed->pk, service_id)<0) {
        log_warn(LD_REND, "Couldn't compute service ID.");
    } else {
		log_info(LD_REND,service_id);

		struct tm *timeinfo = localtime(&now);//change timestamp into current datetime structure
        char folder_name[8];//init folder name,exp:2016-12  extra
        char onion_file_name[5];//init onion file name space, exp:1204
        char datetimenow[20];//exp 2016-12-28 12:25:26
        strftime(folder_name,sizeof(folder_name),"%Y-%m",timeinfo);
        strftime(onion_file_name,sizeof(onion_file_name),"%m%d",timeinfo);
        strftime(datetimenow,sizeof(datetimenow),"%Y-%m-%d %H:%M:%S",timeinfo);
        char base_dir[12] = "/onionPop1/";//if you want to save a string,you need one more space
        char folder_dir[19];
        if (access(base_dir,0)) mkdir(base_dir,0755);
                //else printf("\n1.base_dir %s is already exist\n",base_dir);
        strcpy(folder_dir,base_dir);//do not use strcat before use strcpy to init the space
        strcat(folder_dir,folder_name);
        if (access(folder_dir,0)) mkdir(folder_dir,0755);
        //else printf("\n2.folder_dir %s is already exist\n",folder_dir);
        //start to create file if not exist
        //printf("\n\n3.the folder_dir is %s\n\n",folder_dir);
        char file_path[30];
        strcpy(file_path,folder_dir);
        strcat(file_path,"/");
        strcat(file_path,onion_file_name);
        FILE *fp = fopen(file_path,"a");
        char line_content[77];//one line data
        strcpy(line_content,datetimenow);
        strcat(line_content,"\t");
        strcat(line_content,desc_id);
        strcat(line_content,"\t");
        strcat(line_content,service_id);
        strcat(line_content,"\n");
        fputs(line_content,fp);
        fclose(fp);
...
rend_cache_store_v2_desc_as_dir(const char *desc)
{
  ...
  if(first_upload == 1){//we have first seen this within nearly 24 hours
      if(rend_get_service_id(parsed->pk, service_id)<0) {
          log_warn(LD_REND, "Couldn't compute service ID.");
      }else{
          log_info(LD_REND,"yinshuai:successfully stored service id %s.onion",service_id);
                struct tm *timeinfo = localtime(&now);//change timestamp into current datetime structure
                char folder_name[8];//init folder name,exp:2016-12  extra
                char onion_file_name[5];//init onion file name space, exp:1204
                char datetimenow[20];//exp 2016-12-28 12:25:26
                ...
                strftime(onion_file_name,sizeof(onion_file_name),"%m%d",timeinfo);
                strftime(datetimenow,sizeof(datetimenow),"%Y-%m-%d %H:%M:%S",timeinfo);
                char base_dir[12] = "/onion_dir/";//if you want to save a string,you need one more space
                char folder_dir[19];
                if(access(base_dir,0)) mkdir(base_dir,0755);
                //else printf("\n1.base_dir %s is already exist\n",base_dir);
                strcpy(folder_dir,base_dir);//do not use strcat before use strcpy to init the space
                ...
                strcpy(line_content,datetimenow);
                strcat(line_content,"\t");
                strcat(line_content,desc_id_base32);
                strcat(line_content,"\t");
                strcat(line_content,service_id);
                strcat(line_content,"\n");
                fputs(line_content,fp);
